...
public class CommentReplyAdapter extends RecyclerView.Adapter<CommentReplyAdapter.ViewHolder> {
    ...
    public CommentReplyAdapter(Context context, List<Comment> commentList, long lzid) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
       // CommentBean object = beanList.get(position);
        Comment object=commentList.get(position);
        if (object.getFromId()== lzid)
            holder.btLableLz.setVisibility(View.VISIBLE);
        else
            holder.btLableLz.setVisibility(View.GONE);
        //评论人的昵称
        holder.replayAuthor.setText(object.getUserByFromId().getNickname());
        //第几楼
        holder.replayIndex.setText(position + 1 + "#");
        //评论时间
        if (object.getCreateTime().equals(null))
            holder.replayTime.setText(DateUtils.getFromNowOnTime(object.getCreateTime()));
        else
            holder.replayTime.setText(StampToDate.stampToDate(String.valueOf(object.getCreateTime())));

        RichText.fromMarkdown(object.getCommentContent()).into(holder.markdownText);
        Picasso.get()
                .load(object.getUserByFromId().getIcon())
                .placeholder(R.drawable.image_placeholder)
                .into(holder.articleUserImage);

        //复制评论内容
        holder.markdownText.setOnLongClickListener(view -> {
            String user = object.getUserByFromId().getNickname();
            String content = object.getCommentContent();
        ...
        holder.articleUserImage.setOnClickListener(view -> {
            ...
            intent.putExtra("userid", object.getUserByFromId().getUserId());
